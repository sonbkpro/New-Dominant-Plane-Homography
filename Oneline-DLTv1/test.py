...
def load_model_weights(net, model_path):
    ...
    matched_state_dict = {k: v for k, v in cleaned_state_dict.items() if k in model_dict}
    if not matched_state_dict:
        raise RuntimeError('No matching parameters found in checkpoint: {}'.format(model_path))

    model_dict.update(matched_state_dict)
    net.load_state_dict(model_dict)
    print('Loaded {}/{} tensors from {}'.format(len(matched_state_dict), len(model_dict), model_path))
    return net
...
def test(args):
    ...
    if args.finetune:
        ...
        if model_path is None:
            model_path = os.path.join(exp_name, 'train_log_Oneline-FastDLT/real_models/resnet34_iter_44000.pth')
        print(model_path)
    ...
    for i, batch_value in enumerate(test_loader):
        ...
        h4p = h4p.to(device)
        print_img_1 = print_img_1.to(device)
...
if __name__=="__main__":
    ...
    parser.add_argument('--gpus', type=int, default=1, help='Unused. Select GPU with CUDA_VISIBLE_DEVICES.')
    ...
    parser.add_argument('--pretrained', type=str2bool, default=False, help='Use ImageNet-pretrained backbone?')
    parser.add_argument('--finetune', type=str2bool, default=True, help='Load trained checkpoint?')
    parser.add_argument('--model_path', type=str, default=None, help='Checkpoint path for evaluation.')
